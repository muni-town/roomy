 ...
 */
export async function resolveEntityAvatar(
  db: DbLike,
  entity: string,
): Promise<string | undefined> {
  const row = await db.query(
    "select avatar from comp_info where entity = ?",
  ).get<{ avatar: string | null }>(entity);
 ...
 */
export async function resolveMessageIcon(
  db: DbLike,
  authorDid: string,
  spaceId: string,
): Promise<string | undefined> {
  return (
    (await resolveEntityAvatar(db, authorDid)) ??
 ...
 */
export async function resolveLatestRoomAuthor(
  db: DbLike,
  roomId: string,
): Promise<string | null> {
  const row = await db.query(
    `select author_e.tail as author_did
      ...
      limit 1`,
  ).get<{ author_did: string | null }>(roomId);

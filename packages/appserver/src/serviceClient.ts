...
export function getServiceClient(): Promise<RoomyServiceClient> {
  ...
  if (!UNSAFE_AUTH_TOKEN) {
    return Promise.reject(
      new Error("LEAF_UNSAFE_AUTH_TOKEN is not set; cannot create RoomyServiceClient"),
    );
  }
...
export function getConnectedSpace(streamDid: StreamDid): Promise<ConnectedSpace> {

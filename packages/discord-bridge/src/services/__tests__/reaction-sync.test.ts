...
function reactionAddEvent(roomy: MockRoomyGateway, spaceDid: string): any {
...
function reactionRemoveEvent(roomy: MockRoomyGateway, spaceDid: string): any {
	return roomy.findEvent(
		spaceDid,
...
describe("handleReactionRemove", () => {
	...
	test("RS04: sends removeBridgedReaction and unregisters mapping", async () => {
		...
		expect(event.reactionId).toBe(EXISTING_EVENT_ULID);
	...
	test("RS06: re-add after remove creates fresh mapping", async () => {
		...
		expect(addEvent.id).not.toBe(OLD_EVENT_ULID);

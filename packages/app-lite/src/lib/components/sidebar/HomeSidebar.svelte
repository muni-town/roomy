<script lang="ts">
  ...
  /**
   * Sidebar for the homepage -- lists the user's joined spaces as links
   * with a floating user control bar at the bottom.
   * Can be reused on any page that wants this "your spaces" nav.
   */
...
<div class="flex flex-col h-full">
  <div class="flex-1 min-h-0 overflow-y-auto [mask-image:linear-gradient(to_bottom,black_90%,transparent_100%)]">
    {#if spacesQuery.data}
      {@const joined = spacesQuery.data.spaces.filter((s) => s.isMember)}

      {#if joined.length > 0}
        <section class="flex flex-col gap-1 px-2 py-2 mt-4" aria-label="Your Spaces">
      ...
      {:else}
        <p class="text-sm text-base-400 px-4 pt-4">

...
test.describe('Roomy App - Core Functionality', () => {
  ...
  test('should load the app and initialize workers', async ({ page }) => {
    ...
    const hasBackend = await page.evaluate(() => {
      return typeof (window as any).backend === 'object' &&
        typeof (window as any).backendStatus === 'object';
    });
    expect(hasBackend).toBeTruthy();
  });

  test('should support SharedWorker or fallback to Worker', async ({ page }) => {
    await page.waitForLoadState('domcontentloaded');
    ...
    expect(workerSupport.hasSharedWorker || workerSupport.hasWorker).toBeTruthy();
  ...
  test('should initialize backend worker and respond to ping', async ({ page }) => {
    await page.waitForLoadState('domcontentloaded');
  ...
  test('should initialize SQLite worker and handle basic queries', async ({ page }) => {
    await page.waitForLoadState('domcontentloaded');
  ...
  test('should have proper COOP/COEP headers for SharedArrayBuffer support', async ({ page }) => {
  ...
  test('should have debug helpers available', async ({ page }) => {
    ...
    const debugHelpers = await page.evaluate(() => {
      ...
      return {
        ...
        hasTestSqliteConnection: typeof helpers?.testSqliteConnection === 'function',
      };
    });
  ...
  test('should handle worker status updates', async ({ page }) => {
    ...
    const statusCheck = await page.evaluate(() => {
      ...
      return {
        ...
        isReactive: typeof backendStatus?.subscribe === 'function' ||
  ...
  test('should handle offline/online scenarios gracefully', async ({ page }) => {
    await page.waitForLoadState('domcontentloaded');

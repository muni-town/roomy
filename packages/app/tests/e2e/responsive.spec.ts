...
test.describe('Responsive Design - Cross-Browser Compatibility', () => {
  ...
  test('should load and function across all viewport sizes', async ({ page }) => {
    for (const viewport of viewports) {
      ...
      expect(functionality.workersWorking, `Workers should work on ${viewport.name}`).toBeTruthy();
  ...
  test('should handle very small screen sizes gracefully', async ({ page }) => {
    ...
    try {
      ...
      console.log('Workers may have issues on very small screens:', (error as Error).message);
    }
  });
  ...
  test('should maintain worker performance across screen sizes', async ({ page }) => {
    ...
    const desktopPerf = performanceResults.find(r => r.viewport === 'desktop');

    if (mobilePerf && desktopPerf) {
      // Mobile shouldn't be more than 3x slower than desktop
      const performanceRatio = mobilePerf.avgQueryDuration / desktopPerf.avgQueryDuration;
    ...
    for (const viewport of [
      ...
      const accessibilityCheck = await page.evaluate(() => {
        return {
          ...
          hasProperContrast: window.getComputedStyle(document.body).color !== 'rgb(0, 0, 0)' ||
            window.getComputedStyle(document.body).backgroundColor !== 'rgb(255, 255, 255)',
        };
      });

      console.log(`Accessibility check for ${viewport.name}:`, accessibilityCheck);

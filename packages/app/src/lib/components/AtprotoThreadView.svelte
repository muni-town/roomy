<script lang="ts">
  ...
  async function extractAndLoadThread(content: string) {
    ...
    const blueskyUrlMatch = content.match(/https:\/\/bsky\.app\/profile\/[^\/]+\/post\/([^<\s"]+)/);
    if (blueskyUrlMatch) {
      const postId = blueskyUrlMatch[1];
      // Convert back to AT URI format
      const authorMatch = content.match(/https:\/\/bsky\.app\/profile\/([^\/]+)\/post/);
      if (authorMatch) {
  ...
  async function replyToPost(post: AtprotoThreadPost, rootPost?: AtprotoThreadPost) {
          ...
          <div class="flex items-center gap-4 mb-4">
            {#if threadData.author.avatar}
              <img 
                ...
                alt={threadData.author.displayName || threadData.author.handle}
                class="size-12 rounded-full object-cover"
              />
            {:else}
              <div class="size-12 rounded-full bg-base-300 flex items-center justify-center">
            ...
            <div>
              ...
              <div class="text-sm opacity-70">
                @{threadData.author.handle} • {getRelativeTime(threadData.record.createdAt)}
              </div>
            </div>
          </div>

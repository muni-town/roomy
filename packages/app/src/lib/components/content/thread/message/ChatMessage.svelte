...
{#snippet actionsSnippet()}
  {#if isEditing}
    ...
    <Button
      ...
      title="Save (Enter)"
      onclick={() => saveEditedMessage(editContent)}
    >
      <IconCheck />
    </Button>
  {/if}
...
<style>
  /*
    The rendered message sits directly under the `.prose` wrapper, so Tailwind
    ...
    bubble. The tiptap editor renders paragraphs nested under `.tiptap`
    (inside `#chat-input`), so those resets never reach it and the editor picks
    up an extra ~1.25em margin above its first paragraph and below its last —
    making the edited content look inset vs. the rendered message. Mirror the
    resets here, scoped to the editing editor so the composer (which is not
    inside `.prose`) is unaffected.
  */

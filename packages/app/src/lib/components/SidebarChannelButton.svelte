<script lang="ts">
  ...
  const totalNotifications = $derived((channelNotifications || 0) + (threadNotifications || 0));
</script>
      ...
      {#each channelThreads as thread}
        {#if thread}
          ...
          {@const isThreadNew = (() => {
            ...
            const latestEntries = Object.values(thread?.timeline?.perAccount ?? {}).sort(
              (a, b) => a.madeAt.getTime() - b.madeAt.getTime(),
            );
          ...
          <div class="group flex items-center gap-1 relative">
            <Button.Root
              ...
              class="flex-1 cursor-pointer px-1 dz-btn dz-btn-ghost justify-start border {thread.id === page.params.thread
                ...
                : 'border-transparent'}"
            >
              <h4 class="flex justify-start items-center w-full gap-2 px-2 text-sm">

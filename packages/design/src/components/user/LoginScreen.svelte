<!--
  Presentational login/register shell extracted from app's LoginForm.
  Pure presentation: no $lib imports, no auth side effects.
  Parent owns auth wiring via callbacks.
-->
<script lang="ts">
  ...
  type Props = {
    /** Bindable handle string (already-normalized by parent setter) */
    handle: string;
    /** Bindable email for register flow */
    email?: string;
    /** Bindable password for register flow */
    password?: string;
    /** Bindable tab selection */
    tab?: "Login" | "Register";
    /** Loading flag (disables submit) */
    loading?: boolean;
    /** Optional error message to display under input */
    error?: string | null;
    /** Previously-used login, surfaced as quick-login button */
    lastLogin?: LastLogin;
    /** Suffix appended to handle on registration (e.g. ".roomy.chat") */
    handleSuffix?: string;
    /** Submit handler for login form */
    onLogin?: (evt: Event) => void;
    /** Submit handler for register form */
    onRegister?: (evt: Event) => void;
    /** Click handler for last-login quick button */
    onLastLoginClick?: (evt: Event) => void;
    /** Pass-through props for outer Box */
    [key: string]: unknown;
  };
          ...
          <div class="text-xs">
            <span class="text-stone-400"
              ...
              <a
                ...
                class="text-accent-600 dark:text-accent-400">Atmosphere</a
              >.</span
            >
          </div></span
        >
      ...
      <div class="my-2 w-full pb-2">
        ...
        {#if lastLogin?.handle}
          <datalist id="past-handles">
            <option value={lastLogin.handle}></option>
          </datalist>
        {/if}
        {#if error}
          <p
  ...
  {:else if tab == "Register"}
    ...
    <form class="flex flex-col gap-2" onsubmit={onRegister}>
      ...
      <div class="flex items-center relative h-9">
        ...
        />
        <span class="absolute right-4 opacity-80">{handleSuffix}</span>
      </div>

...
import { w as Ae } from "./Dxu-ImQV.js";
...
window.fetch = (e, t) => ((e instanceof Request ? e.method : t?.method || "GET") !== "GET" && G.delete(Se(e)), kt(e, t));
...
const U = globalThis.__sveltekit_1c3v7kt?.base ?? "", xt = globalThis.__sveltekit_1c3v7kt?.assets ?? U, Pt = "1742334473158", Ke = "sveltekit:snapshot", We = "sveltekit:scroll", Ye = "sveltekit:states", Ct = "sveltekit:pageurl", V = "sveltekit:history", W = "sveltekit:navigation", j = { tap: 1, hover: 2, viewport: 3, eager: 4, off: -1, false: -1 }, X = location.origin;
function Je(e) {
...
Jt ? (S = { data: {}, form: null, error: null, params: {}, route: { id: null }, state: {}, status: -1, url: new URL("https://example.com") }, Y = { current: null }, _e = { current: false }) : (S = new class {
  #e = T({});
  get data() {
    return x(this.#e);
  }
...
const en = /* @__PURE__ */ new Set(["icon", "shortcut icon", "apple-touch-icon"]), D = He(We) ?? {}, J = He(Ke) ?? {}, O = { url: De({}), page: De({}), navigating: Ae(null), updated: Nt() };
function Ue(e) {
  D[e] = Ee();
}
function tn(e, t) {
...
let y = { branch: [], error: null, url: null }, Te = false, se = false, Be = true, z = false, q = false, rt = false, xe = false, at, k, I, $;
const K = /* @__PURE__ */ new Set();
async function Sn(e, t, n) {
  document.URL !== location.href && (location.href = location.href), v = e, await e.hooks.init?.(), Le = Tt(e), C = document.documentElement, ke = t, be = e.nodes[0], re = e.nodes[1], be(), re(), k = history.state?.[V], I = history.state?.[W], k || (k = I = Date.now(), history.replaceState({ ...history.state, [V]: k, [W]: I }, ""));
  const r = D[k];
  r && (history.scrollRestoration = "manual", scrollTo(r.x, r.y)), n ? await pn(ke, n) : await un(v.hash ? mn(new URL(location.href)) : location.href, { replaceState: true }), hn();
}
function nn() {
...
function it(e, t, n) {
  ...
  r && r.remove(), Object.assign(S, e.props.page), at = new v.root({ target: t, props: { ...e.props, stores: O, components: oe }, hydrate: n, sync: false }), st(I);
...
function ie({ url: e, params: t, branch: n, status: r, error: a, route: s, form: i }) {
  ...
  const c = { type: "loaded", state: { url: e, params: t, branch: n, error: a, route: s }, props: { constructors: Ht(n).map((l) => l.node.component), page: Oe(S) } };
  ...
  return (!y.url || e.href !== y.url.href || y.error !== a || i !== void 0 && i !== S.form || u) && (c.props.page = { error: a, params: t, route: { id: s?.id ?? null }, state: {}, status: r, url: new URL(e), form: i ?? null, data: u ? f : S.data }), c;
}
...
function ct(e, t, n) {
  ...
  const s = se ? St(r, a.href, t) : At(r, t);
...
async function lt({ id: e, invalidating: t, url: n, params: r, route: a, preload: s }) {
  ...
  let p = false;
  const _ = f.map((g, w) => {
    const b = y.branch[w], A = !!g?.[0] && (b?.loader !== g[1] || qe(p, d, h, l, b.server?.uses, r));
    return A && (p = true), A;
  });
  if (_.some(Boolean)) {
    try {
  ...
  const E = f.map(async (g, w) => {
    if (!g) return;
    const b = y.branch[w], A = m?.[w];
    if ((!A || A.type === "skip") && g[1] === b?.loader && !qe(R, d, h, l, b.universal?.uses, r)) return b;
    if (R = true, A?.type === "error") throw A;
    return Ce({ loader: g[1], url: n, params: r, route: a, parent: async () => {
      const he = {};
      for (let pe = 0; pe < w; pe += 1) Object.assign(he, (await E[pe])?.data);
      return he;
    }, server_data_node: Ne(A === void 0 && g[0] ? { type: "skip" } : A ?? null, g[0] ? b?.server : void 0) });
  });
  ...
  for (let g = 0; g < f.length; g += 1) if (f[g]) try {
    ...
    let b = ne(w), A;
    if (m?.includes(w)) b = w.status ?? b, A = w.error;
    else if (w instanceof fe) A = w.body;
    else {
      if (await O.updated.check()) return await et(), await M(n);
      A = await B(w, { params: r, url: n, route: { id: a.id } });
    }
    const Z = await on(g, L, i);
    return Z ? ie({ url: n, params: r, branch: L.slice(0, Z.idx).concat(Z.node), status: b, error: A, route: a }) : await ut(n, { id: a.id }, A, b);
  }
  else L.push(void 0);
...
async function ee({ type: e, url: t, popped: n, keepfocus: r, noscroll: a, replace_state: s, state: i = {}, redirect_count: o = 0, nav_token: c = {}, accept: f = Ve, block: u = Ve }) {
  ...
  if (nn(), Ue(p), ot(_), m.props.page.url.pathname !== t.pathname && (t.pathname = m.props.page.url.pathname), i = n ? n.state : i, !n) {
    const g = s ? 0 : 1, w = { [V]: k += g, [W]: I += g, [Ye]: i };
    (s ? history.replaceState : history.pushState).call(history, w, "", t), s || tn(k, I);
  }
  ...
  !r && !L && gn(), Be = true, m.props.page && Object.assign(S, m.props.page), z = false, e === "popstate" && st(I), l.fulfil(void 0), H.forEach((g) => g(l.navigation)), O.navigating.set(Y.current = null);
}
...
function hn() {
  history.scrollRestoration = "manual", addEventListener("beforeunload", (t) => {
    ...
    if (u !== void 0 && h) {
      ...
      if (q = true, Ue(k), e(r), !o.replace_state) return;
      q = false;
    }
    ...
    if (t.state?.[V]) {
      ...
      if (i === I && (rt || o)) {
        a !== S.state && (S.state = a), e(s), D[k] = Ee(), r && scrollTo(r.x, r.y), k = n;
        return;
      }
      const f = n - k;
    ...
    q && (q = false, history.replaceState({ ...history.state, [V]: ++k, [W]: I }, "", location.href));
  });
  ...
  function e(t) {
    y.url = S.url = t, O.page.set(Oe(S)), O.page.notify();
  }
}
